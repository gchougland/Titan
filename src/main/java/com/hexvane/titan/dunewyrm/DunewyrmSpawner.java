...
public final class DunewyrmSpawner {
    ...
    public static Result spawn(@Nonnull final Store<EntityStore> store,
                               ...
                               final float yaw) {
        return spawn(store, position, yaw, ThreadLocalRandom.current().nextLong(), true, null, null);
    }
    ...
    public static Result spawn(@Nonnull final Store<EntityStore> store,
        ...
        if (bodyPrefs != null && !bodyPrefs.isEmpty()) {
            for (final DunewyrmSegment body : bodyPrefs) {
                chain.add(new DunewyrmSegment(DunewyrmSegmentRole.BODY, body.getPrefabKey(), body.isMirrored(),
                    body.getHealth() > 0 ? body.getHealth() : DunewyrmTuning.SEGMENT_HEALTH));
            }
        } else {
    ...
    public static int spawnSegmentVoxels(@Nonnull final Store<EntityStore> store,
        ...
        for (final PrefabVoxels.Voxel voxel : voxels.getVoxels()) {
            ...
            holder.addComponent(DunewyrmPartComponent.getComponentType(),
                new DunewyrmPartComponent(root, segmentIndex, local, voxel.rotation(), partScale));
    ...
    public static void layoutAlongPath(@Nonnull final DunewyrmComponent worm) {
        ...
        final float groundY = (float) worm.getHeadPosition().y;
        ...
        pts[0] = new Vector3d(
            ...
            groundY + rise - worm.getTunnelDepth(),
        ...
        for (int c = 1; c < chainLen; c++) {
            ...
            // Smooth S: high at the head, concave mid-neck, flat by the rear bodies.
            final double s = t * t * (3.0 - 2.0 * t);           // smoothstep 0→1
            final double sCurve = (1.0 - s) * (1.0 - s) * (1.0 + 2.0 * s); // 1 at head, ease to 0
            final double height = rise * sCurve;
            // Lean is signed: negative = recoil (head/neck back), positive = strike forward.
            // Extra back-arch on the mid neck so the silhouette is an S, not a straight ramp.
            final double neckArch = rise > 0.05f ? -rise * 0.22 * Math.sin(Math.PI * (1.0 - s)) : 0.0;
            ...
            // Face toward the head along the welded link.
            ...
            pitches[c] = horiz < 1e-5 ? (ty >= 0 ? (float) (Math.PI * 0.5) : (float) (-Math.PI * 0.5))
        ...
        // Head pitch follows the first link so the skull stays on the S.
        if (chainLen > 1) {
            pitches[0] = pitches[1];
            yaws[0] = yaws[1];
        }
        ...
        if (headSeg != null) {
            ...
            if (tongue != null) {
                ...
                tongue.getPosition().set(
                    headSeg.getPosition().x + hfx * reach * cosP,
                    headSeg.getPosition().y + 0.4 + reach * sinP,
                    headSeg.getPosition().z + hfz * reach * cosP);
                tongue.setYaw(hy + (float) Math.PI);
                tongue.setPitch(hp);
            }
        }
    }
    ...
    /**
     * Rebuilds visual head/tail (and optionally jaw/tongue) voxels after a structural change.
     */
    public static void rebuildVisuals(@Nonnull final Store<EntityStore> store,
        ...
        for (int i = 0; i < worm.getSegments().size(); i++) {
            final DunewyrmSegmentRole role = worm.getSegments().get(i).getRole();
            if (role == DunewyrmSegmentRole.BODY) continue;
